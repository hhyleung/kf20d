...
async function loadFridgeStock() {
  try {
    const { data, error } = await sb.from('fridge_stock').select('*').order('last_updated', { ascending: false });
...
async function loadChores() {
  try {
    const { data, error } = await sb.from('chores').select('*').order('next_due_date');
    if (error) throw error;
    chores = data || [];
  } catch (err) {
...
async function loadChangeLogs() {
  try {
    const { data, error } = await sb.from('change_log').select('*').order('next_change_due');
    if (error) throw error;
    changeLogs = data || [];
  } catch (err) {
...
async function loadBills() {
  try {
    const { data, error } = await sb.from('bills').select('*').order('next_bill_date');
    if (error) throw error;
    bills = data || [];
  } catch (err) {
...
async function loadPlants() {
  try {
    const { data, error } = await sb.from('plants').select('*').order('plant_name');
    if (error) throw error;
    plants = data || [];
  } catch (err) {
...
async function loadPlantHistory() {
  try {
    const { data, error } = await sb.from('plant_history').select('*').order('event_date', { ascending: false });
    if (error) throw error;
    plantHistory = data || [];
  } catch (err) {
...
async function setupRealtime() {
  ...
  subscribeToTable('planthistory', () => loadPlantHistory().then(renderPlants));
...
async function savePlant(plant) {
  ...
  if (error) throw error;
  await loadPlants(); await loadPlantHistory();
}
...
async function deletePlant(plantId) {
  const sb = await initSupabase();
  const { error: error1 } = await sb.from('planthistory').delete().eq('plantid', plantId);
  const { error: error2 } = await sb.from('plants').delete().eq('id', plantId);
...
const fridgeCategoryOrder = ['Carbs', 'Veg', 'Protein', 'Fruits', 'Others', 'Raw'];
...
// ============================================================
const LIST_META_KEYS = ['fridge_stock', 'chores', 'bills', 'change_log', 'plants', 'notes'];
let listMetadata = {};
LIST_META_KEYS.forEach(k => { listMetadata[k] = null; });
...
async function touchMetadata(listName) {
  ...
  try {
    const sb = await initSupabase();
    await sb.from('listmetadata').upsert({ listname: listName, lastupdated: ts, userid: sb.auth.getUser().data.user.id });
...
function calcChoreNextDue(lastdone, interval) { return addDays(lastdone, interval); }
function calcChangeLogNextDue(lastchanged, interval) { return addMonths(lastchanged, interval); }
function calcBillNextDate(lastbilldate, interval) { return addMonths(lastbilldate, interval); }
...
function groupByCategory(items) {
  const grouped = {};
  fridgeCategoryOrder.forEach(c => { grouped[c] = []; });
  items.forEach(item => {
    grouped[fridgeCategoryOrder.includes(item.category) ? item.category : 'Others'].push(item);
  });
  return grouped;
}
...
function fridgeItemRow(item, showZero = false) {
  ...
  const dateStr = dateVal
    ...
    : '';
  const portionsDisplay = item.portions === 0 && showZero
...
function buildFridgeHTML(showZero) {
  ...
  fridgeCategoryOrder.forEach(cat => {
    ...
    const wide = !alwaysHalfWidth.includes(cat) && (catItems.length >= 3 || forceFullWidth.includes(cat));
    const cls = wide ? 'col-full' : 'col-half';
    html += catItems.length === 0
...
function renderFridgeStock() {
  ...
  if (currentFullSection === 'fridgestock') {
    document.getElementById('fullSectionContent').innerHTML = buildFridgeHTML(true);
  }
}
...
function openDetail(itemId) {
  ...
  document.getElementById("editExpiry").value = formatDateInput(item.expirydate);
  document.getElementById("editLastUpdated").textContent = item.lastupdated ? formatShortDate(item.lastupdated) : "";
  document.getElementById("detailTitle").textContent = item.itemname.toUpperCase();

  const expectedAuto = item.createdat && item.shelflifedays ? 
    addDays(item.createdat, item.shelflifedays) : null;
...
function buildChoresHTML() {
  return '<ul class="item-list">' + chores.map(c => {
...
function renderChores() {
  ...
  if (currentFullSection === 'chores') {
    document.getElementById('fullSectionContent').innerHTML = buildChoresHTML();
  }
}
...
function openChoreDetail(choreId) {
  ...
  document.getElementById("choreLastdone").value = formatDateInput(chore.lastdone);
  document.getElementById("choreInterval").value = chore.interval;
  document.getElementById("choreNextdue").value = formatDateInput(chore.nextdue);

  const expectedAuto = chore.lastdone && chore.interval ? 
    calcChoreNextDue(chore.lastdone, chore.interval) : null;
  choreNextDueManuallySet = !!chore.nextdue && chore.nextdue !== expectedAuto;

  document.getElementById("choreDetailTitle").textContent = chore.taskname.toUpperCase();
...
function refreshChoreNextDueIfNeeded() {
  ...
  if (lastdone && interval) {
    document.getElementById('choreNextdue').value = calcChoreNextDue(lastdone, interval);
    if (autoLabel) autoLabel.style.display = 'inline-block';
  } else {
    if (autoLabel) autoLabel.style.display = 'none';
  }
...
function buildChangeLogsHTML() {
  return '<ul class="item-list">' + changeLogs.map(c => {
    const dueClass = getDueClass(c.nextdue);
    return `
...
function renderChangeLogs() {
  document.getElementById('changelogContent').innerHTML = buildChangeLogsHTML();
  if (currentFullSection === 'changelog') {
    document.getElementById('fullSectionContent').innerHTML = buildChangeLogsHTML();
  }
}
...
function openChangeLogDetail(clId) {
  ...
  document.getElementById("changelogLastchanged").value = formatDateInput(cl.lastchanged);
  document.getElementById("changelogInterval").value = cl.interval;
  document.getElementById("changelogNextdue").value = formatDateInput(cl.nextdue);

  const expectedAuto = cl.lastchanged && cl.interval ? 
    calcChangeLogNextDue(cl.lastchanged, cl.interval) : null;
  changelogNextDueManuallySet = !!cl.nextdue && cl.nextdue !== expectedAuto;

  document.getElementById("changelogDetailTitle").textContent = cl.itemname.toUpperCase();
...
function refreshChangelogNextDue() {
  ...
  if (lastchanged && interval) {
    document.getElementById('changelogNextdue').value = calcChangeLogNextDue(lastchanged, interval);
    if (autoLabel) autoLabel.style.display = 'inline-block';
  } else {
    if (autoLabel) autoLabel.style.display = 'none';
  }
...
function buildBillsHTML() {
  return '<ul class="item-list">' + bills.map(b => {
...
function renderBills() {
  ...
  if (currentFullSection === 'bills') {
    document.getElementById('fullSectionContent').innerHTML = buildBillsHTML();
  }
}
...
function openBillDetail(billId) {
  ...
  document.getElementById("billLastbilldate").value = formatDateInput(bill.lastbilldate);
  document.getElementById("billInterval").value = bill.interval;
  document.getElementById("billNextbilldate").value = formatDateInput(bill.nextbilldate);

  const expectedAuto = bill.lastbilldate && bill.interval ? 
    calcBillNextDate(bill.lastbilldate, bill.interval) : null;
  billNextDateManuallySet = !!bill.nextbilldate && bill.nextbilldate !== expectedAuto;

  document.getElementById("billDetailTitle").textContent = bill.billname.toUpperCase();
...
function refreshBillNextDate() {
  ...
  if (lastdate && interval) {
    document.getElementById('billNextbilldate').value = calcBillNextDate(lastdate, interval);
    if (autoLabel) autoLabel.style.display = 'inline-block';
  } else {
    if (autoLabel) autoLabel.style.display = 'none';
  }
...
function buildPlantsHTML(showArchived = false) {
  ...
  return '<ul class="item-list">' + visible.map(p => `
...
function renderPlants() {
  ...
  if (currentFullSection === 'plants') {
    document.getElementById('fullSectionContent').innerHTML = buildPlantsHTML(true);
  }
}
...
function openPlantDetail(plantId) {
  ...
  document.getElementById('plantDetailTitle').textContent = plant.plantname.toUpperCase();
  document.getElementById('pdPlantNameInput').value = plant.plantname;
  document.getElementById('pdSaveNameBtn').dataset.id = plantId;
  document.getElementById('pdStartDate').textContent = plant.startingdate ? formatShortDate(plant.startingdate) : '—';
  document.getElementById('pdPotSize').textContent = plant.potsize ? plant.potsize + ' cm' : '—';
  document.getElementById('pdLastWatered').textContent = plant.lastwatered ? formatShortDate(plant.lastwatered) : '—';
  document.getElementById('pdLastFertilised').textContent = plant.lastfertilised ? formatShortDate(plant.lastfertilised) : '—';
  document.getElementById('pdFertiliserUsed').textContent = plant.lastfertiliser || '—';
  ...
  document.getElementById('pdLatestNote').textContent = noteText || 'No notes';

  const archiveBtn = document.getElementById('pdArchiveBtn');
  archiveBtn.dataset.id = plantId;
  archiveBtn.textContent = plant.archived ? 'Unarchive Plant' : 'Archive Plant';
  ...
  if (!history.length) {
    tbody.innerHTML = '<tr><td colspan="7" style="color:var(--text-secondary);font-style:italic;text-align:center;padding:1.4rem;">No history yet</td></tr>';
  } else {
...
function openAddPlantModal() {
  ...
  document.getElementById('addPlantStartDate').value = new Date().toISOString().slice(0, 10);
...
function openPlantEventModal(plantId) {
  const plant = plants.find(p => p.id === plantId);
  if (!plant) return;
  document.getElementById('plantEventTitle').textContent = plant.plantname.toUpperCase();
...
function buildNotesHTML() {
  return '<ul class="item-list">' + notes.map(n => `
    <li class="item-row">
...
function renderNotes() {
  ...
  if (currentFullSection === 'notes') {
    document.getElementById('fullSectionContent').innerHTML = buildNotesHTML();
  }
}
...
function buildFullSectionHTML(section) {
  switch (section) {
    case 'fridgestock': return buildFridgeHTML(true);
    case 'chores':      return buildChoresHTML();
    case 'changelog':   return buildChangeLogsHTML();
    case 'bills':       return buildBillsHTML();
    case 'plants':      return buildPlantsHTML(true);
    case 'notes':       return buildNotesHTML();
    default:            return '';
  }
}
...
function setupPanelClicks() {
  ...
  document.querySelectorAll('.panel-header h3').forEach(h3 => {
    h3.addEventListener('click', e => {
      ...
      document.getElementById('sectionTitle').textContent = h3.textContent.toUpperCase();
      currentFullSection = section;
      document.getElementById('fullSectionContent').innerHTML = buildFullSectionHTML(section);
...
document.addEventListener('click', async e => {
  ...
  if (action === 'fridge-portions') {
    ...
    fridgeItems = fridgeItems.map(it => 
      it.id === id ? { ...it, portions: newPortions, lastupdated: today } : it
    );
    touchMetadata('fridgestock');
    renderFridgeStock();
    
    try {
      await sb.from('fridgestock')
  ...
  if (action === 'done') {
    ...
    chores = chores.map(c => c.id !== id ? c : { ...c, lastdone: today, nextdue });
    touchMetadata('chores');
    renderChores();
    
    try {
      await sb.from('chores').update({ lastdone: today, nextdue }).eq('id', id);
    } catch (err) {
      console.error(err);
      loadChores();
    }
  ...
  if (action === 'changelog-done') {
    ...
    changeLogs = changeLogs.map(c => c.id !== id ? c : { ...c, lastchanged: today, nextdue });
    touchMetadata('changelog');
    renderChangeLogs();
    
    try {
      await sb.from('changelog').update({ lastchanged: today, nextdue }).eq('id', id);
    } catch (err) {
      console.error(err);
      loadChangeLogs();
    }
  ...
  if (action === 'paid') {
    ...
    bills = bills.map(b => b.id !== id ? b : { ...b, lastbilldate: newLast, nextbilldate: newNext });
    touchMetadata('bills');
    renderBills();
    
    try {
      await sb.from('bills').update({ lastbilldate: newLast, nextbilldate: newNext }).eq('id', id);
    } catch (err) {
      console.error(err);
      loadBills();
    }
  ...
  if (action === 'plant-save-name') {
    const newName = document.getElementById('pdPlantNameInput').value.trim();
    if (!newName) return;
    try {
      await sb.from('plants').update({ plantname: newName }).eq('id', id);
      document.getElementById('plantDetailTitle').textContent = newName.toUpperCase();
  ...
  if (action === 'plant-delete') {
    const plantId = id;
    if (confirm('Delete this plant and all its history permanently? This cannot be undone.')) {
      try {
  ...
  if (detailRow) { openDetail(detailRow.dataset.openDetail); return; }

  const choreRow = e.target.closest('[data-open-chore]');
  if (choreRow) { openChoreDetail(choreRow.dataset.openChore); return; }

  const changelogRow = e.target.closest('[data-open-changelog]');
  if (changelogRow) { openChangeLogDetail(changelogRow.dataset.openChangelog); return; }

  const billRow = e.target.closest('[data-open-bill]');
  if (billRow) { openBillDetail(billRow.dataset.openBill); return; }

  const openPlantEl = e.target.closest('[data-open-plant]');
  if (openPlantEl) { openPlantDetail(openPlantEl.dataset.openPlant); return; }
});
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  ['fullSectionModal', 'detailModal', 'addModal', 'plantEventModal', 'plantDetailModal', 'addPlantModal',
   'choreDetailModal', 'changelogDetailModal', 'billDetailModal', 'addNoteModal'].forEach(id => {
  ...
  // --- Fridge: add button ---
  document.querySelectorAll('.panel[data-section="fridgestock"] .add-btn').forEach(btn =>
    btn.addEventListener('click', e => { e.stopPropagation(); openAddModal(); }));

  document.getElementById('deleteItemBtn')?.addEventListener('click', () =>
    deleteFridgeItem(document.getElementById('editId').value));

  ['editCreatedAt', 'editShelflife'].forEach(id =>
    document.getElementById(id)?.addEventListener('change', () => { expiryManuallySet = false; recalcExpiry(); }));
  ...
  ['addCreatedAt', 'addShelflife'].forEach(id =>
    document.getElementById(id)?.addEventListener('change', () => { addExpiryManuallySet = false; recalcAddExpiry(); }));
  ...
  document.getElementById('editForm')?.addEventListener('submit', async e => {
    ...
    const shelfVal = fd.get('shelflifedays') ? parseInt(fd.get('shelflifedays'), 10) : null;
    const expirydate = expiryManuallySet ? fd.get('expirydate') : (createdVal && shelfVal ? addDays(createdVal, shelfVal) : null);
  ...
  document.getElementById('addForm')?.addEventListener('submit', async e => {
    ...
    const createdVal = fd.get('createdat');
    const shelfVal = fd.get('shelflifedays') ? parseInt(fd.get('shelflifedays'), 10) : null;
    const expirydate = addExpiryManuallySet ? fd.get('expirydate') : (createdVal && shelfVal ? addDays(createdVal, shelfVal) : null);
  ...
  // --- Chores: add button ---
  document.querySelectorAll('.panel[data-section="chores"] .add-btn').forEach(btn =>
    btn.addEventListener('click', e => {
      e.stopPropagation();
      editingChoreId = null; choreNextDueManuallySet = false;
      ...
      document.getElementById('choreDetailTitle').textContent = 'ADD CHORE';
      document.getElementById('choreDeleteBtn').style.display = 'none';
      openModal('choreDetailModal');
    }));

  ['choreLastdone', 'choreInterval'].forEach(id =>
    document.getElementById(id)?.addEventListener('change', () => { choreNextDueManuallySet = false; refreshChoreNextDueIfNeeded(); }));
  ...
  document.getElementById('choreForm')?.addEventListener('submit', async e => {
    ...
    const lastdone = document.getElementById('choreLastdone').value || null;
    const interval = parseInt(document.getElementById('choreInterval').value, 10) || null;
    const nextdue = choreNextDueManuallySet ? document.getElementById('choreNextdue').value : (lastdone && interval ? calcChoreNextDue(lastdone, interval) : null);
  ...
  // Chore delete
  document.getElementById('choreDeleteBtn')?.addEventListener('click', async () => {
  ...
  document.querySelectorAll('.panel[data-section="changelog"] .add-btn').forEach(btn =>
    btn.addEventListener('click', e => {
      e.stopPropagation();
      editingChangeLogId = null; changelogNextDueManuallySet = false;
      ...
      document.getElementById('changelogNextdueAutoLabel').style.display = 'none';
      document.getElementById('changelogDetailTitle').textContent = 'ADD CHANGE LOG';
      document.getElementById('changelogDeleteBtn').style.display = 'none';
      openModal('changelogDetailModal');
    }));

  ['changelogLastchanged', 'changelogInterval'].forEach(id =>
    document.getElementById(id)?.addEventListener('change', () => { changelogNextDueManuallySet = false; refreshChangelogNextDue(); }));
  document.getElementById('changelogNextdue')?.addEventListener('input', () => {
  ...
  document.getElementById('changelogForm')?.addEventListener('submit', async e => {
    ...
    const lastchanged = document.getElementById('changelogLastchanged').value || null;
    const interval = parseInt(document.getElementById('changelogInterval').value, 10) || null;
    const nextdue = changelogNextDueManuallySet ? document.getElementById('changelogNextdue').value : (lastchanged && interval ? calcChangeLogNextDue(lastchanged, interval) : null);
  ...
  // Change Log delete
  document.getElementById('changelogDeleteBtn')?.addEventListener('click', async () => {
  ...
  // --- Bills: add button ---
  document.querySelectorAll('.panel[data-section="bills"] .add-btn').forEach(btn =>
    btn.addEventListener('click', e => {
      e.stopPropagation();
      editingBillId = null; billNextDateManuallySet = false;
      ...
      document.getElementById('billNextbilldateAutoLabel').style.display = 'none';
      document.getElementById('billDetailTitle').textContent = 'ADD BILL';
      document.getElementById('billDeleteBtn').style.display = 'none';
      openModal('billDetailModal');
    }));

  ['billLastbilldate', 'billInterval'].forEach(id =>
    document.getElementById(id)?.addEventListener('change', () => { billNextDateManuallySet = false; refreshBillNextDate(); }));
  document.getElementById('billNextbilldate')?.addEventListener('input', () => {
  ...
  document.getElementById('billForm')?.addEventListener('submit', async e => {
    ...
    const lastbilldate = document.getElementById('billLastbilldate').value || null;
    const interval = parseInt(document.getElementById('billInterval').value, 10) || null;
    const nextbilldate = billNextDateManuallySet ? document.getElementById('billNextbilldate').value : (lastbilldate && interval ? calcBillNextDate(lastbilldate, interval) : null);
  ...
  // Bill delete
  document.getElementById('billDeleteBtn')?.addEventListener('click', async () => {
  ...
  // --- Plants: add button ---
  document.querySelectorAll('.panel[data-section="plants"] .add-btn').forEach(btn =>
    btn.addEventListener('click', e => {
      e.stopPropagation();
      openAddPlantModal();
    }));
  ...
  document.getElementById('addPlantForm')?.addEventListener('submit', async e => {
    ...
    const startDate = document.getElementById('addPlantStartDate').value;
    if (!name) return;
    const plant = { plantname: name, startingdate: startDate, archived: false };
    try {
      const sb = await initSupabase();
      await sb.from('plants').insert([{ ...plant, userid: sb.auth.getUser().data.user.id }]);
      await loadPlants(); loadPlantHistory();
      closeModal('addPlantModal');
    } catch (err) { alert('Add failed: ' + err.message); }
  });

  // --- Plants: fertilised checkbox toggle ---
  document.getElementById('fertilisedCheck')?.addEventListener('change', e => {
    document.getElementById('fertiliserSelectWrap').style.display = e.target.checked ? 'block' : 'none';
  });

  // Plants event form
  document.getElementById('plantEventForm')?.addEventListener('submit', async e => {
    e.preventDefault();
    const fd = new FormData(e.target);
    ...
    const resolvedPotsize = !isNaN(potsizeVal) && potsizeVal > 0 ? potsizeVal : null;
    ...
    try {
      ...
      await updatePlant(plantId, {
        ...
        ...(fertilised && { lastfertilised: today, lastfertiliser: fertiliserUsed }),
  ...
  // --- Notes: add button ---
  document.querySelectorAll('.panel[data-section="notes"] .add-btn').forEach(btn =>
    btn.addEventListener('click', e => {
      e.stopPropagation();
  ...
  document.getElementById('addNoteForm')?.addEventListener('submit', async e => {
    ...
    try {
      const sb = await initSupabase();
      await sb.from('notes').insert([{ content, userid: sb.auth.getUser().data.user.id }]);
      await loadNotes();
